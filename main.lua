...
if getgenv().F_A then
  return getgenv().F_A.Notify(nil, "Loaded", "fates admin is already loaded... use 'killscript' to kill", nil);
end
...
GetPlayer = function(str)
  ...
  if (not next(Players)) then
	  getgenv().F_A.utilsNotify(LocalPlayer, "Fail", ("Couldn't find player %s"):format(str));
  end
  return Players
end
...
local function utilsTween(Object, Style, Direction, Time, Goal)
...
local function utilsMultColor3(Color, Delta)
  return Color3.new(math.clamp(Color.R * Delta, 0, 1), math.clamp(Color.G * Delta, 0, 1), math.clamp(Color.B * Delta, 0, 1))
end

local function utilsClick(Object, Goal) -- utilsClick(Object, "BackgroundColor3")
  local Hover = {
	  [Goal] = utilsMultColor3(Object[Goal], 0.9)
  }

  local Press = {
	  [Goal] = utilsMultColor3(Object[Goal], 1.2)
  }
  ...
  Connections["ObjectMouseEnter" .. #Connections] = Object.MouseEnter:Connect(function()
	  utilsTween(Object, "Sine", "Out", .5, Hover)
  end)

  Connections["ObjectMouseLeave" .. #Connections] = Object.MouseLeave:Connect(function()
	  utilsTween(Object, "Sine", "Out", .5, Origin)
  end)

  Connections["ObjectMouseButton1Down" .. #Connections] = Object.MouseButton1Down:Connect(function()
	  utilsTween(Object, "Sine", "Out", .3, Press)
  end)

  Connections["ObjectMouseButton1Up" .. #Connections] = Object.MouseButton1Up:Connect(function()
	  utilsTween(Object, "Sine", "Out", .4, Hover)
  end)
end

local function utilsBlink(Object, Goal, Color1, Color2) -- utilsClick(Object, "BackgroundColor3", NormalColor, OtherColor)
  local Normal = {
	  [Goal] = Color1
  }
  ...
  local Tween = utilsTween(Object, "Sine", "Out", .5, Blink)
  Tween.Completed:Wait()

  local Tween = utilsTween(Object, "Sine", "Out", .5, Normal)
  Tween.Completed:Wait()
end

local function utilsHover(Object, Goal)
  local Hover = {
	  [Goal] = utilsMultColor3(Object[Goal], 0.9)
  }
...
local function utilsDraggable(Ui, DragUi)
  local DragSpeed = 0
  local StartPos
...
local function utilsSmoothScroll(content, SmoothingFactor) -- by Elttob
...
local function utilsTweenAllTransToObject(Object, Time, BeforeObject) -- max transparency is max object transparency, swutched args bc easier command
  local Descendants = Object:GetDescendants()
  local OldDescentants = BeforeObject:GetDescendants()
  local Tween -- to use to wait

  Tween = utilsTween(Object, "Sine", "Out", Time, {
...
local function utilsSetAllTrans(Object)
...
local function utilsTweenAllTrans(Object, Time)
  local Tween -- to use to wait
...
local function utilsNotify(Caller, Title, Message, Time)
  if (not Caller or Caller == LocalPlayer) then
	  ...
	  local function TweenDestroy()
		  local Tween = utilsTweenAllTrans(Clone, .25)
	  ...
	  utilsSetAllTrans(Clone)
	  utilsClick(Clone.Close, "TextColor3")
	  ...
	  coroutine.wrap(function()
		  local Tween = utilsTweenAllTransToObject(Clone, .5, Notification)
...
local function utilsMatchSearch(String1, String2) -- utilsMatchSearch("pog", "poggers") - true; utilsMatchSearch("poz", "poggers") - false
  return String1 == string.sub(String2, 1, #String1)
end

local function utilsStringFind(Table, String)
  for _, v in ipairs(Table) do
	  if (utilsMatchSearch(String, v)) then
		  return v
	  end
  end
end

local function utilsGetPlayerArgs(Arg)
  Arg = Arg:lower()
  local SpecialCases = {"all", "others", "claimed", "random", "me", "nearest", "farthest"}

  return utilsStringFind(SpecialCases, Arg) or (function()
	  for _, v in ipairs(Players:GetPlayers()) do
		  local Name = string.lower(v.Name)

		  if (utilsMatchSearch(Arg, Name)) then
			  return Name
		  end
	  end
  end)()
...
local function utilsToolTip(Object, Message)
  local Clone
...
local function utilsClearAllObjects(Object)
...
local function utilsRainbow(TextObject)
...
local function utilsLocate(Player, Color)
...
local function utilsAddTag(Tag)
...
local function utilsTextFont(Text, RGB)
	  ...
	  Function = function()
		  for i, v in next, options do
			  if (type(v) == 'function' and v() == false) then
				  utilsNotify(LocalPlayer, "Fail", ("You are missing something that is needed for this command (%s)"):format(debug.getinfo(v).namewhat));
				  return nil
			  elseif (type(v) == 'number' and CommandRequirements[v].Func() == false) then
				  utilsNotify(LocalPlayer, "Fail", CommandRequirements[v].Message);
				  return nil
			  end
		  end
		  return func
...
---@param Hum any
---@return table
local function ReplaceHumanoid(Hum)
...
---@param Tbl table
local function AddPlayerConnection(Player, Connection, Tbl)
  if (Connections) then
	  if (Tbl) then
...
---@param Connection any
---@param Tbl table
local function AddConnection(Connection, Tbl)
  if (Connections) then
	  if (Tbl) then
		  Tbl[#Tbl + 1] = Connection
	  else
  ...
  for i, v in next, Plugins do
	  ...
	  if (Executed and not Err) then
		  local Executed, Err = pcall(function()
			  ...
			  utilsHover(Clone, "BackgroundColor3");
			  utilsToolTip(Clone, Cmd.Name .. "\n" .. Cmd.Description);
			  Clone.CommandText.RichText = true
			  Clone.CommandText.Text = ("%s %s %s"):format(Cmd.Name, next(Cmd.Aliases) and ("(%s)"):format(table.concat(Cmd.Aliases, ", ")) or "", utilsTextFont("[PLUGIN]", {77, 255, 255}))
			  Clone.Name = Cmd.Name
			  Clone.Visible = true
...
AddCommand("commandcount", {"cc"}, "shows you how many commands there is in fates admin", {}, function(Caller)
  utilsNotify(Caller, "Amount of Commands", ("There are currently %s commands."):format(#table.filter(CommandsTable, function(i,v)
  ...
  coroutine.wrap(function()
	  for i, v in next, Target do
			  if (GetCharacter(v)) then
				  if (isSat(v)) then
					  utilsNotify(Caller or LocalPlayer, nil, v.Name .. " is sitting down, could not kill");
					  do break end
				  end
				  ...
				  utilsNotify(Caller or LocalPlayer, "Fail", v.Name .. " is dead or does not have a root part, could not kill.");
			  end
	  end
  end)()
	  ...
	  for i, v in next, Target do
		  repeat
			  if (GetCharacter(v)) then
				  ...
				  utilsNotify(Caller or LocalPlayer, "Fail", v.Name .. " is dead or does not have a root part, could not bring.");
			  end
		  until true
	  end
	  wait(.2);
  ...
  coroutine.wrap(function()
	  for i, v in next, Target do
		  repeat
			  if (GetCharacter(v)) then
				  if (isSat(v)) then
					  utilsNotify(Caller or LocalPlayer, nil, v.Name .. " is sitting down, could not bring");
					  do break end
				  end
  ...
  coroutine.wrap(function()
	  for i, v in next, Target do
		  repeat
			  if (GetCharacter(v)) then
				  if (isSat(v)) then
					  utilsNotify(Caller or LocalPlayer, nil, v.Name .. " is sitting down, could not void");
					  do break end
				  end
				  ...
				  utilsNotify(Caller or LocalPlayer, "Fail", v.Name .. " is dead or does not have a root part, could not void.");
			  end
		  until true
	  end
  end)();
...
AddCommand("savetools", {"st"}, "saves your tools", {1,3}, function(Caller, Args)
  ...
  utilsNotify(Caller, nil, "Tools are now saved");
...
AddCommand("chatmute", {"cmute"}, "mutes a player in your chat", {"1"}, function(Caller, Args)
  ...
  for i, v in next, Target do
	  ...
	  utilsNotify(Caller, "Command", ("%s is now muted on your chat"):format(v.Name));
  end
end)
...
AddCommand("unchatmute", {"uncmute"}, "unmutes a player in your chat", {"1"}, function(Caller, Args)
  ...
  for i, v in next, Target do
	  MuteRequest:InvokeServer(v.Name);
	  utilsNotify(Caller, "Command", ("%s is now unmuted on your chat"):format(v.Name));
  end
end)
...
AddCommand("delete", {}, "puts a players character in lighting", {"1"}, function(Caller, Args)
  ...
  for i, v in next, Target do
	  if (v.Character) then
		  ...
		  utilsNotify(Caller, "Command", v.Name .. "'s character is now parented to lighting");
	  end
  end
end)
...
AddCommand("recover", {"undelete"}, "removes a players character parented from lighting", {"1"}, function(Caller, Args)
  ...
  for i, v in next, Target do
	  if (v.Character and v.Character.Parent == Lighting) then
		  ...
		  utilsNotify(Caller, "Command", v.Name .. "'s character is now in workspace");
	  else
		  utilsNotify(Caller, "Command", v.Name .. "'s character is not removed");
	  end
  end
end)
...
AddCommand("load", {"loadstring"}, "loads whatever you want", {"1"}, function(Caller, Args)
  ...
  local Success, Err = pcall(function()
	  loadstring(("%s\n%s\n%s"):format("local oldprint=print print=function(...)getgenv().F_A.utilsNotify(game.Players.LocalPlayer,'Command',table.concat({...},' '))return oldprint(...)end", Code, "print = oldprint"))();
  end)
  ...
  local Success, Err = pcall(function()
	  loadstring(("%s\n%s\n%s"):format([[
		  ...
		  print = function(...)
			  ...
			  getgenv().F_A.utilsNotify(game.Players.LocalPlayer,'Command',table.concat({...},' '))
			  return oldprint(...)
		  end
		  warn = print
...
AddCommand("firetouchinterests", {"fti"}, "fires all the touch interests", {3}, function(Caller, Args)
  ...
  for i, v in next, Workspace:GetDescendants() do
	  if (v:IsA("TouchTransmitter")) then
		  firetouchinterest(utilsGetRoot(LocalPlayer.Character), v.Parent, 0);
		  wait();
		  firetouchinterest(utilsGetRoot(LocalPlayer.Character), v.Parent, 1);
		  amount = amount + 1
		  if (howmany and amount == tonumber(howmany)) then break; end
	  end
  end
  ...
  for i, v in next, Target do
	  for i2, v2 in next, v.Character:GetDescendants() do
		  if (v2:IsA("Sound") and v2.Parent.Parent:IsA("Tool")) then
			  ...
			  utilsNotify(Caller, "Command", ("Audio Id (%s) copied to clipboard"):format(AudioId));
		  end
	  end
  end
end)
...
AddCommand("unlocate", {"nolocate"}, "disables location for a player", {"1"}, function(Caller, Args)
  ...
  for i, v in next, Locating do
	  for i2, v2 in next, Target do
		  if (i == v2.Name) then
			  v:Destroy();
			  utilsNotify(Caller, "Command", v2.Name .. " is no longer being located");
		  else
			  utilsNotify(Caller, "Command", v2.Name .. " isn't being located");
		  end
	  end
  end
end)
...
AddCommand("esp", {}, "turns on player esp", {}, function(Caller, Args, Tbl)
  ...
  table.forEach(Players:GetPlayers(), function(i,v)
	  Tbl.Billboards[#Tbl.Billboards + 1] = utilsLocate(v);
  ...
  PlayerAddedConnection = Players.PlayerAdded:Connect(function(Player)
	  ...
	  local Esp = Player.CharacterAdded:Connect(function()
		  Player.Character:WaitForChild("HumanoidRootPart");
		  Player.Character:WaitForChild("Head");
		  Tbl.Billboards[#Tbl.Billboards + 1] = utilsLocate(Player);
	  end)
	  Tbl[#Tbl + 1] = Esp
...
AddCommand("age", {}, "ages a player", {"1"}, function(Caller, Args)
  ...
  for i, v in next, Target do
	  ...
	  utilsNotify(Caller, "Command", ("%s's age is %s (%s)"):format(v.Name, AccountAge, CreatedAt));
  end
end)
...
AddCommand("antikick", {}, "client sided bypasses to kicks", {}, function()
  ...
  mt.__namecall = newcclosure(function(self, ...)
	  ...
	  if (method == "kick") then
		  utilsNotify(Caller or LocalPlayer, "Attempt to kick", ("attempt to kick with message \"%s\""):format(tostring(args[1])));
		  return wait(9e9);
	  end
	  return oldnc(self, ...);
  end)
...
AddCommand("chatlogs", {"clogs"}, "enables chatlogs", {}, function()
  ...
  utilsClearAllObjects(ChatLogs.Frame.List)
  ChatLogs.Visible = true

  local Tween = utilsTweenAllTransToObject(ChatLogs, .25, ChatLogsTransparencyClone)
  ...
  utilsTween(ChatLogs.Frame.List, "Sine", "Out", .25, {
...
AddCommand("globalchatlogs", {"globalclogs"}, "enables globalchatlogs", {}, function()
  ...
  utilsClearAllObjects(GlobalChatLogs.Frame.List);
  GlobalChatLogs.Visible = true

  local Tween = utilsTweenAllTransToObject(GlobalChatLogs, .25, GlobalChatLogsTransparencyClone);
  ...
  utilsTween(GlobalChatLogs.Frame.List, "Sine", "Out", .25, {
	  ScrollBarImageTransparency = 0
  });
...
AddCommand("httplogs", {"httpspy"}, "enables httpspy", {}, function()
  ...
  utilsClearAllObjects(HttpLogs.Frame.List)
  HttpLogs.Visible = true

  local Tween = utilsTweenAllTransToObject(HttpLogs, .25, HttpLogsTransparencyClone)
  ...
  utilsTween(HttpLogs.Frame.List, "Sine", "Out", .25, {
	  ScrollBarImageTransparency = 0
  })
end)
...
if (hookfunction and syn) then
  local AddLog = function(reqType, url, body)
	  ...
	  Clone.Text = ("%s\nUrl: %s%s\n"):format(utilsTextFont(reqType .. " Detected (time: " .. tostring(os.date("%X")) ..")", {255, 165, 0}), url, body and ", Body: " .. utilsTextFont(body, {255, 255, 0}) or "");
	  Clone.RichText = true
	  Clone.Visible = true
	  Clone.TextTransparency = 1
...
AddCommand("noclip", {}, "noclips your character", {3}, function(Caller, Args, Tbl)
  ...
  Tbl[1] = Noclipping
  utilsNotify(Caller, "Command", "noclip enabled");
  GetHumanoid().Died:Wait();
...
AddCommand("whisper", {}, "whispers something to another user", {"2"}, function(Caller, Args)
  ...
  for i, v in next, Target do
	  ...
	  utilsNotify(Caller or LocalPlayer, "Command", "Message sent to " .. v.Name);
  end
end)
...
AddCommand("serverhop", {"sh"}, "switches servers (optional: min or max)", {{"min", "max"}}, function(Caller, Args)
  if (Caller == LocalPlayer) then
	  utilsNotify(Caller or LocalPlayer, nil, "Looking for servers...");
...
AddCommand("whitelist", {"wl"}, "whitelists a user so they can use commands", {"1"}, function(Caller, Args)
  ...
  for i, v in next, Target do
	  ...
	  utilsNotify(v, "Whitelisted", ("You (%s) are whitelisted to use commands"):format(v.Name));
  end
end)
...
AddCommand("exceptions", {}, "blocks user from being used in stuff like kill all", {"1"}, function(Caller, Args)
  ...
  for i, v in next, Target do
	  ...
	  utilsNotify(Caller, "Command", v.Name .. " is added to the exceptions list");
  end
end)
...
AddCommand("noexception", {}, "removes user from exceptions list", {"1"}, function(Caller, Args)
  for i2, v2 in next, Exceptions do
	  ...
	  utilsNotify(Caller, "Command", Args[1] .. " is removed from the exceptions list");
  end
end)
...
AddCommand("commands", {"cmds"}, "shows you all the commands listed in fates admin", {}, function()
  ...
  utilsTweenAllTransToObject(Commands, .25, CommandsTransparencyClone);
...
AddCommand("setprefix", {}, "changes your prefix", {"1"}, function(Caller, Args)
  ...
  if (PrefixToSet:match("%A")) then
	  Prefix = PrefixToSet
	  utilsNotify(Caller, "Command", ("your new prefix is now '%s'"):format(PrefixToSet));
...
AddCommand("setcommandbarprefix", {"setcprefix"}, "sets your command bar prefix to whatever you input", {}, function()
  ...
  local CloseNotif = utilsNotify(LocalPlayer, "New Prefix", "Input the new prefix you would like to have", 7);
end)
...
AddCommand("widebar", {}, "widens the command bar (toggle)", {}, function(Caller, Args)
  ...
  if (not Draggable) then
	  utilsTween(CommandBar, "Quint", "Out", .5, {
...
AddCommand("draggablebar", {"draggable"}, "makes the command bar draggable", {}, function(Caller)
  ...
  utilsDraggable(CommandBar);
  local TransparencyTween = CommandBarOpen and utilsTweenAllTransToObject or utilsTweenAllTrans
	  ...
	  if (table.find(AdminUsers, plr) or plr == LocalPlayer) then
		  ...
		  if (LoadedCommand) then
			  if (LoadedCommand.ArgsNeeded > #Args) then
				  return utilsNotify(plr, "Error", ("Insuficient Args (you need %d)"):format(LoadedCommand.ArgsNeeded))
			  end

			  local Success, Err = pcall(function()
				  local Executed = LoadedCommand.Function()(plr, Args, LoadedCommand.CmdExtra);
				  if (Executed) then
					  utilsNotify(plr, "Command", Executed);
				  end
			  ...
			  utilsNotify(plr, "Error", ("couldn't find the command %s"):format(Command));
		  end
	  end
  end)
end
...
Stats.Visible = false
utilsSetAllTrans(CommandBar)
utilsSetAllTrans(ChatLogs)
utilsSetAllTrans(GlobalChatLogs)
utilsSetAllTrans(HttpLogs);
...
utilsDraggable(Commands)
utilsDraggable(ChatLogs)
utilsDraggable(GlobalChatLogs)
utilsDraggable(HttpLogs);
...
Connections.UI.CommandBarInput = UserInputService.InputBegan:Connect(function(Input, GameProccesed)
  ...
  elseif (not GameProccesed and ChooseNewPrefix) then
	  ...
	  utilsNotify(LocalPlayer, "New Prefix", "Your new prefix is: " .. tostring(Input.KeyCode):split(".")[3]);
	  ChooseNewPrefix = false
	  if (writefile) then
		  utilsNotify(LocalPlayer, nil, "use command saveprefix to save your prefix");
	  end
  end
end)

-- smooth scroll commands
utilsSmoothScroll(Commands.Frame.List, .14)
...
for _, v in next, CommandsTable do -- auto size
  if (not Commands.Frame.List:FindFirstChild(v.Name)) then
	  ...
	  utilsHover(Clone, "BackgroundColor3") -- add tooltip
	  utilsToolTip(Clone, v.Name .. "\n" .. v.Description)
...
utilsClick(Commands.Close, "TextColor3")
...
utilsSetAllTrans(Commands)
utilsClick(ChatLogs.Clear, "BackgroundColor3")
utilsClick(ChatLogs.Save, "BackgroundColor3")
utilsClick(ChatLogs.Toggle, "BackgroundColor3")
utilsClick(ChatLogs.Close, "TextColor3")

utilsClick(GlobalChatLogs.Clear, "BackgroundColor3")
utilsClick(GlobalChatLogs.Save, "BackgroundColor3")
utilsClick(GlobalChatLogs.Toggle, "BackgroundColor3")
utilsClick(GlobalChatLogs.Close, "TextColor3")

utilsClick(HttpLogs.Clear, "BackgroundColor3")
utilsClick(HttpLogs.Save, "BackgroundColor3")
utilsClick(HttpLogs.Toggle, "BackgroundColor3")
utilsClick(HttpLogs.Close, "TextColor3")

-- close tween commands
Connections.CommandsClose = Commands.Close.MouseButton1Click:Connect(function()
  local Tween = utilsTweenAllTrans(Commands, .25)
...
Connections.UI.ChatLogsClose = ChatLogs.Close.MouseButton1Click:Connect(function()
  local Tween = utilsTweenAllTrans(ChatLogs, .25)

  Tween.Completed:Wait()
  ChatLogs.Visible = false
end)
Connections.UI.GlobalChatLogsClose = GlobalChatLogs.Close.MouseButton1Click:Connect(function()
  local Tween = utilsTweenAllTrans(GlobalChatLogs, .25)
...
Connections.UI.ChatLogsSave = ChatLogs.Save.MouseButton1Click:Connect(function()
  ...
  utilsNotify(LocalPlayer, "Saved", "Chat logs saved!");
end)
...
Connections.CommandBarChanged = CommandBar.Input:GetPropertyChangedSignal("Text"):Connect(function() -- make it so that every space a players name will appear
  ...
  if (not CommandsTable[Args[1]]) then
	  for _, v in next, CommandsTable do
		  ...
		  if (utilsMatchSearch(Args[1], CommandName)) then -- better search
		  ...
		  for _, v2 in next, Aliases do
			  if (utilsMatchSearch(Args[1], v2)) then
				  FoundAlias = true
				  Prediction.Text = v2
	  ...
	  if (i > 1 and v ~= "") then
		  ...
		  if (#CommandArgs >= 1) then
			  for i2, v2 in next, CommandArgs do
				  if (v2:lower() == "player") then
					  Predict = utilsGetPlayerArgs(v) or Predict;
				  else
					  Predict = utilsMatchSearch(v, v2) and v2 or Predict
				  end
			  end
		  else
			  Predict = utilsGetPlayerArgs(v) or Predict;
		  ...
		  if (next(split)) then
			  for i2, v2 in next, split do
				  if (i2 > 1 and v2 ~= "") then
					  local PlayerName = utilsGetPlayerArgs(v2)
...
Connections.CommandBar = CommandBar.Input.FocusLost:Connect(function()
  ...
  if (not Draggable) then
	  utilsTweenAllTrans(CommandBar, .5)
	  utilsTween(CommandBar, "Quint", "Out", .5, {
		  Position = UDim2.new(0.5, WideBar and -200 or -100, 1, 5); -- tween 5
	  })
  end
...
local PlayerAdded = function(plr)
  ...
  if (Tag and plr ~= LocalPlayer) then
	  Tag.Player = plr
	  utilsNotify(LocalPlayer, "Admin", ("%s (%s) has joined"):format(Tag.Name, Tag.Tag));
	  utilsAddTag(Tag);
	  coroutine.wrap(function()
...
utilsNotify(LocalPlayer, "Loaded", ("script loaded in %.3f seconds"):format((tick() or os.clock()) - start));
utilsNotify(LocalPlayer, "Welcome", "'cmds' to see all of the commands");
